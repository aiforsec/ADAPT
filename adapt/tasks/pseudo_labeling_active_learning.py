...
class ActiveLearning(BaseTask):
    ...
    def active_learning_iterations(self):
        ...
        for month in unique_months:
            ...
            ### Active Learning Sample Selection First
            selected_indices = self.model.sample_active_learning(X_val_month,
                                                                 self.cfg.EXPERIMENT.ACTIVE_LEARNING_SAMPLES)

            ### Add Active Learning Samples to Training Set Before Retraining
            end_idx = start_idx + selected_indices.shape[0]
            train_x[start_idx:end_idx] = X_val_month_orig[selected_indices]
            train_y[start_idx:end_idx] = y_val_month[selected_indices]
            start_idx = end_idx

            ### Pseudo-Labeled Sample Selection After Active Learning
            index_pseudo, y_pseudo = self.sample_pseudo_labeled_data(X_val_month)

            # Remove pseudo-labeled samples that overlap with active learning samples
            index_pseudo = np.setdiff1d(index_pseudo, selected_indices)

            X_pseudo = X_val_month_orig[index_pseudo]
            y_pseudo = y_val_month[index_pseudo]


            ### Combine Active Learning and Pseudo-Labeled Data for Training
            if self.cfg.EXPERIMENT.PSEUDO_LABELING_SOURCE_FREE:
                ...
                X_combined = np.concatenate((train_x[:start_idx], X_pseudo), axis=0)
                y_combined = np.concatenate((train_y[:start_idx], y_pseudo), axis=0)

                if self.cfg.DATASET.STANDARDIZE:
                    self.standard_scaler = StandardScaler()
                    X_combined = self.standard_scaler.fit_transform(X_combined)

            print(start_idx, X_combined.shape)
            X_combined, y_combined = self.get_merged_dataset(X_combined, y_combined)

            ### Retrain the Classifier After Adding Active Learning and Pseudo-Labeled Data
            if self.cfg.EXPERIMENT.PSEUDO_LABELING_SOURCE_FREE:
            ...
            if not self.cfg.EXPERIMENT.VALIDATION_MODE:
                self.model.save_model(
                    os.path.join(self.out_dir, 'model_' + str(self.cfg.EXPERIMENT.SEED) + '_' + str(month)))
    ...
    def run(cls, cfg):
        ...
        if cfg.EXPERIMENT.VALIDATION_MODE:
            ...
            for seed in range(cfg.EXPERIMENT.START_SEED_TEST, cfg.EXPERIMENT.START_SEED_TEST + 1):
...
def run():
    ...
    _cfg.EXPERIMENT.OUT_DIR = 'output-pseudo-label-al'
    ...
    if args.test:
        _cfg.EXPERIMENT.OUT_DIR = 'test-output-pseudo-label-al'
        if args.source_free:
            _cfg.EXPERIMENT.OUT_DIR += '-source_free'
